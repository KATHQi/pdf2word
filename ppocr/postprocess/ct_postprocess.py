...
class CTPostProcess(object):
    ...
    def __init__(self, min_score=0.56, min_area=10, box_type='poly', **kwargs):
    ...
    def __call__(self, preds, batch):
        ...
        for idx in range(batch_size):
            ...
            for i in range(1, label_num):
                ind = (label_kernel == i)
                if ind.sum(
                ) < 8:  # pixel number less than 10, treated as background
                    label_kernel[ind] = 0
            ...
            for i in range(1, label_num):
                ...
                if self.box_type == 'quad':
                    ...
                    if type(box) is list:
                        box = np.array(box)
                    box = order_points_clockwise(box)
                    box = clip_det_res(box, img_size[0], img_size[1])
                    rect_width = int(np.linalg.norm(box[0] - box[1]))
                    rect_height = int(np.linalg.norm(box[0] - box[3]))
                    # if rect_width <= 3 or rect_height <= 3:
                    #     continue
            ...
            # bboxes=filter_tag_det_res(bboxes, img_shape)
