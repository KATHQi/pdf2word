# Copyright (c) 2020 PaddlePaddle Authors. All Rights Reserved.
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

import os
import sys

__dir__ = os.path.dirname(os.path.abspath(__file__))
sys.path.append(__dir__)
sys.path.insert(0, os.path.abspath(os.path.join(__dir__, '..')))
sys.path.insert(0, os.path.abspath(os.path.join(__dir__, '../..')))

os.environ["FLAGS_allocator_strategy"] = 'auto_growth'
import cv2
import json
import copy
import logging
import numpy as np
import time
import tools.infer.predict_rec as predict_rec
import tools.infer.predict_det as predict_det
import tools.infer.utility as utility
from tools.infer.predict_system import sorted_boxes
from ppocr.utils.utility import get_image_file_list, check_and_read
from ppocr.utils.logging import get_logger
from ppstructure.table.matcher import TableMatch
from ppstructure.table.table_master_match import TableMasterMatcher
from ppstructure.utility import parse_args
import ppstructure.table.predict_structure as predict_strture

logger = get_logger()


def expand(pix, det_box, shape):
    x0, y0, x1, y1 = det_box
    #     print(shape)
    h, w, c = shape
    tmp_x0 = x0 - pix
    tmp_x1 = x1 + pix
    tmp_y0 = y0 - pix
    tmp_y1 = y1 + pix
    x0_ = tmp_x0 if tmp_x0 >= 0 else 0
    x1_ = tmp_x1 if tmp_x1 <= w else w
    y0_ = tmp_y0 if tmp_y0 >= 0 else 0
    y1_ = tmp_y1 if tmp_y1 <= h else h
    return x0_, y0_, x1_, y1_


class TableSystem(object):
    def __init__(self, args, text_detector=None, text_recognizer=None):
        self.args = args
        if not args.show_log:
            logger.setLevel(logging.INFO)
        benchmark_tmp = False
        if args.benchmark:
            benchmark_tmp = args.benchmark
            args.benchmark = False
        self.text_detector = predict_det.TextDetector(copy.deepcopy(
            args)) if text_detector is None else text_detector
        self.text_recognizer = predict_rec.TextRecognizer(copy.deepcopy(
            args)) if text_recognizer is None else text_recognizer
        if benchmark_tmp:
            args.benchmark = True
        self.table_structurer = predict_strture.TableStructurer(args)
        if args.table_algorithm in ['TableMaster']:
            self.match = TableMasterMatcher()
            # print('no filter')
        else:
            self.match = TableMatch(filter_ocr_result=True)
            # print('with filter')

        self.predictor, self.input_tensor, self.output_tensors, self.config = utility.create_predictor(
            args, 'table', logger)

    def __call__(self, img, return_ocr_result_in_table=False):
        result = dict()
        time_dict = {'det': 0, 'rec': 0, 'table': 0, 'all': 0, 'match': 0}
        start = time.time()
        structure_res, elapse = self._structure(copy.deepcopy(img))
        # print('\n\n===========structure_res==========\n')
        # print(structure_res)
        # print('\n\n===========structure_res==========\n')
        result['cell_bbox'] = structure_res[1].tolist()
        time_dict['table'] = elapse

        dt_boxes, rec_res, det_elapse, rec_elapse = self._ocr(
            copy.deepcopy(img))
        time_dict['det'] = det_elapse
        time_dict['rec'] = rec_elapse

        result['boxes'] = [x.tolist() for x in dt_boxes] if dt_boxes is not None else []
        result['rec_res'] = rec_res if rec_res is not None else []

        tic = time.time()
        pred_html = self.match(structure_res, dt_boxes, rec_res)
        toc = time.time()
        time_dict['match'] = toc - tic
        result['html'] = pred_html
        result['structure_res'] = structure_res
        end = time.time()
        time_dict['all'] = end - start
        # with open(os.path.join(args.output, 'rec_res.txt'),'w', encoding='utf-8') as f:
        #     f.write(json.dumps( rec_res ,ensure_ascii=False))

        # with open(os.path.join(args.output, 'dt_boxes.txt'),'w', encoding='utf-8') as f:
        #     f.write(json.dumps( dt_boxes.tolist() ,ensure_ascii=False))
        #     # for ii in dt_boxes:
        #         # print('aaaaa:::\n',ii)
        #         # f.write(json.dumps( ii ,ensure_ascii=False))
        # with open(os.path.join(args.output, 'structure_res.txt'),'w', encoding='utf-8') as f:
        #     for ii in structure_res:
        #         if type(ii) ==type([1,2]):
        #             f.write(json.dumps( ii,ensure_ascii=False)+'\n')
        #         else:
        #             f.write(json.dumps( ii.tolist() ,ensure_ascii=False)+'\n')
        return result, time_dict

    def _structure(self, img):
        structure_res, elapse = self.table_structurer(copy.deepcopy(img))
        return structure_res, elapse

    def _ocr(self, img):

        h, w = img.shape[:2]
        dt_boxes, det_elapse = self.text_detector(copy.deepcopy(img))
        # print('^^^^^^',len(dt_boxes))
        dt_boxes = sorted_boxes(dt_boxes)

        r_boxes = []
        for box in dt_boxes:
            x_min = max(0, box[:, 0].min() - 1)
            x_max = min(w, box[:, 0].max() + 1)
            y_min = max(0, box[:, 1].min() - 1)
            y_max = min(h, box[:, 1].max() + 1)
            box = [x_min, y_min, x_max, y_max]
            r_boxes.append(box)
        dt_boxes = np.array(r_boxes)
        logger.debug("dt_boxes num : {}, elapse : {}".format(
            len(dt_boxes), det_elapse))
        if dt_boxes is None:
            return None, None

        img_crop_list = []
        for i in range(len(dt_boxes)):
            det_box = dt_boxes[i]
            x0, y0, x1, y1 = expand(2, det_box, img.shape)
            text_rect = img[int(y0):int(y1), int(x0):int(x1), :]
            img_crop_list.append(text_rect)
        rec_res, rec_elapse = self.text_recognizer(img_crop_list)
        # for i in rec_res:
        #     print(i[0])
        logger.debug("rec_res num  : {}, elapse : {}".format(
            len(rec_res), rec_elapse))
        return dt_boxes, rec_res, det_elapse, rec_elapse


def to_excel(html_table, excel_path):
    from tablepyxl import tablepyxl
    tablepyxl.document_to_xl(html_table, excel_path)


def save_batch_results(batch_results, output_dir):
    """
    保存所有文件的识别结果为 JSON 文件
    
    Args:
        batch_results: 包含所有文件识别结果的列表
        output_dir: 输出目录
    """
    import json
    
    # 将不可序列化的对象转换为字符串
    serializable_results = []
    for result in batch_results:
        serialized = {
            'image_file': result['image_file'],
            'basename': result['basename'],
            'html': result['html'],
            'cell_bbox': result['cell_bbox'],
            'boxes': [box.tolist() if hasattr(box, 'tolist') else box for box in result.get('boxes', [])],
            'rec_res': result.get('rec_res', []),
            'excel_path': result['excel_path'],
            'img_save_path': result['img_save_path'],
            'predict_time': result['predict_time'],
            'time_dict': result['time_dict']
        }
        serializable_results.append(serialized)
    
    # 保存为 JSON
    json_path = os.path.join(output_dir, 'batch_results.json')
    with open(json_path, 'w', encoding='utf-8') as f:
        json.dump(serializable_results, f, ensure_ascii=False, indent=2)
    logger.info(f"Batch results saved to {json_path}")


def save_batch_results_csv(batch_results, output_dir):
    """
    保存所有文件的识别结果为 CSV 文件（简表）
    
    Args:
        batch_results: 包含所有文件识别结果的列表
        output_dir: 输出目录
    """
    import csv
    
    csv_path = os.path.join(output_dir, 'batch_results.csv')
    with open(csv_path, 'w', newline='', encoding='utf-8') as f:
        fieldnames = [
            'image_file',
            'basename',
            'cell_bbox_count',
            'predict_time',
            'excel_path',
            'img_save_path'
        ]
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()
        
        for result in batch_results:
            row = {
                'image_file': result['image_file'],
                'basename': result['basename'],
                'cell_bbox_count': len(result['cell_bbox']),
                'predict_time': f"{result['predict_time']:.3f}s",
                'excel_path': result['excel_path'],
                'img_save_path': result['img_save_path']
            }
            writer.writerow(row)
    logger.info(f"Batch results CSV saved to {csv_path}")



def main(args):
    image_file_list = get_image_file_list(args.image_dir)
    image_file_list = image_file_list[args.process_id::args.total_process_num]
    os.makedirs(args.output, exist_ok=True)

    table_sys = TableSystem(args)
    img_num = len(image_file_list)

    # 初始化批量结果存储
    batch_results = []  # 存储所有文件的识别结果
    
    f_html = open(
        os.path.join(args.output, 'show.html'), mode='w', encoding='utf-8')
    f_html.write('<html>\n<body>\n')
    f_html.write('<table border="1">\n')
    f_html.write(
        "<meta http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8\" />"
    )
    f_html.write("<tr>\n")
    f_html.write('<td>img name\n')
    f_html.write('<td>ori image</td>')
    f_html.write('<td>table html</td>')
    f_html.write('<td>cell box</td>')
    f_html.write("</tr>\n")

    for i, image_file in enumerate(image_file_list):
        logger.info("[{}/{}] {}".format(i, img_num, image_file))
        img, flag, _ = check_and_read(image_file)
        excel_path = os.path.join(
            args.output, os.path.basename(image_file).split('.')[0] + '.xlsx')
        if not flag:
            img = cv2.imread(image_file)
        if img is None:
            logger.error("error in loading image:{}".format(image_file))
            continue
        starttime = time.time()
        pred_res, time_dict = table_sys(img, return_ocr_result_in_table=True)
        pred_html = pred_res['html']
        # logger.info(pred_html)
        to_excel(pred_html, excel_path)
        logger.info('excel saved to {}'.format(excel_path))
        elapse = time.time() - starttime
        logger.info("Predict time : {:.3f}s".format(elapse))

        if len(pred_res['cell_bbox']) > 0 and len(pred_res['cell_bbox'][
                0]) == 4:
            img = predict_strture.draw_rectangle(image_file,
                                                 pred_res['cell_bbox'])
        else:
            img = utility.draw_boxes(img, pred_res['cell_bbox'])
        img_save_path = os.path.join(args.output, os.path.basename(image_file))
        print('【aaaaaa】img_save_path',img_save_path)
        cv2.imwrite(img_save_path, img)

        # 保存此文件的识别结果
        file_result = {
            'image_file': image_file,
            'basename': os.path.basename(image_file),
            'html': pred_html,
            'cell_bbox': pred_res['cell_bbox'],
            'boxes': pred_res.get('boxes', []),
            'rec_res': pred_res.get('rec_res', []),
            'excel_path': excel_path,
            'img_save_path': img_save_path,
            'predict_time': elapse,
            'time_dict': time_dict
        }
        batch_results.append(file_result)

        # 追加写入 structure_res.txt
        structure_res_path = os.path.join(args.output, 'structure_res.txt')
        with open(structure_res_path, 'a', encoding='utf-8') as f_res:
            for ii in pred_res['structure_res']:
                if isinstance(ii, list):
                    f_res.write(json.dumps(ii, ensure_ascii=False) + '\n')
                else:
                    f_res.write(json.dumps(ii.tolist(), ensure_ascii=False) + '\n')

        f_html.write("<tr>\n")
        f_html.write(f'<td> {os.path.basename(image_file)} <br/>\n')
        f_html.write(f'<td><img src="{image_file}" width=640></td>\n')
        f_html.write('<td><table  border="1">' + pred_html.replace(
            '<html><body><table>', '').replace('</table></body></html>', '') +
                     '</table></td>\n')
        f_html.write(
            f'<td><img src="{os.path.basename(image_file)}" width=640></td>\n')
        f_html.write("</tr>\n")
    f_html.write("</table>\n")
    f_html.close()

    # 保存批量结果为 JSON 文件
    save_batch_results(batch_results, args.output)
    
    # 保存全局 bbox.json (包含 cell_bbox, boxes, rec_res)
    global_bbox_path = os.path.join(args.output, 'bbox.json')
    global_bbox_data = []
    for res in batch_results:
        global_bbox_data.append({
            'image_file': res['image_file'],
            'cell_bbox': res['cell_bbox'],               # predict_structure 识别的单元格框
            'det_boxes': res['boxes'],                  # OCR 检测框
            'rec_res': res['rec_res']                    # OCR 识别文字
        })
    with open(global_bbox_path, 'w', encoding='utf-8') as f_bbox:
        json.dump(global_bbox_data, f_bbox, ensure_ascii=False, indent=2)
    logger.info(f"Global bbox results saved to {global_bbox_path}")

    # 保存批量结果为 CSV 文件（简表）
    save_batch_results_csv(batch_results, args.output)

    if args.benchmark:
        table_sys.table_structurer.autolog.report()
    
    logger.info("All {} images processed. Results saved to {}".format(img_num, args.output))


if __name__ == "__main__":
    args = parse_args()
    if args.use_mp:
        import subprocess
        p_list = []
        total_process_num = args.total_process_num
        for process_id in range(total_process_num):
            cmd = [sys.executable, "-u"] + sys.argv + [
                "--process_id={}".format(process_id),
                "--use_mp={}".format(False)
            ]
            p = subprocess.Popen(cmd, stdout=sys.stdout, stderr=sys.stdout)
            p_list.append(p)
        for p in p_list:
            p.wait()
    else:
        main(args)
