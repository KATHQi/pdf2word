...
def get_table_dimensions(table_soup):
    ...
    for row in rows:
        ...
        row_col_count = sum(int(col.attrs.get('colspan', 1)) for col in cols)
    ...
    def handle_table(self, html, doc):
        ...
        for index, row in enumerate(rows):
            ...
            for col in cols:
                ...
                while docx_cell.text != '' and cell_col < cols_len - 1:  # Skip the merged cell
                    cell_col += 1
                    docx_cell = table.cell(cell_row, cell_col)

                # Clamp rowspan/colspan to actual table bounds to avoid IndexError
                actual_rowspan = min(rowspan, len(rows) - cell_row)
                actual_colspan = min(colspan, cols_len - cell_col)
                cell_to_merge = table.cell(cell_row + actual_rowspan - 1,
                                           cell_col + actual_colspan - 1)
